...
class StoryRemoteMediator(
    ...
    override suspend fun load(loadType: LoadType, state: PagingState<Int, Story>): MediatorResult {
        ...
        try {
            val responseData = apiService.getStories(StoryRepository.TOKEN, page, state.config.pageSize).listStory
            val endOfPaginationReached = responseData.isEmpty()

            val insertStory = responseData.map {
                Story(
                    it.id!!,
                    it.name,
                    it.description,
            ...
            database.withTransaction {
                ...
                database.storyDao().insertStory(insertStory)
            }
